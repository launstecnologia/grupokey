...
write_log('=== DEBUG PRODUTOS NA VIEW EDIT ===', 'app.log');
write_log('Estabelecimento ID: ' . ($establishment['id'] ?? 'N/A'), 'app.log');
write_log('ProductData: ' . json_encode($productData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), 'app.log');

// Função auxiliar para verificar se um produto está selecionado
function isProductSelected($productId, $productData) {
    if (empty($productData)) {
        return false;
    }
    
    // IDs possíveis para CDX/EVO
    $cdxEvoIds = ['prod-pagseguro', 'prod-subaquirente'];
    // IDs possíveis para CDC
    $cdcIds = ['prod-brasil-card', 'prod-cdc', 'prod-brasilcard'];
    
    // Primeiro, verificar se há produtos em 'other' (prioridade)
    if (isset($productData['other']) && is_array($productData['other'])) {
        foreach ($productData['other'] as $otherProduct) {
            $otherProductType = $otherProduct['product_type'] ?? '';
            
            // Verificar correspondência exata
            if ($otherProductType === $productId) {
                return true;
            }
            
            // Verificar se é CDX/EVO
            if (in_array($productId, $cdxEvoIds) && in_array($otherProductType, $cdxEvoIds)) {
                return true;
            }
            
            // Verificar se é CDC
            if (in_array($productId, $cdcIds) && in_array($otherProductType, $cdcIds)) {
                return true;
            }
        }
    }
    
    // Depois verificar nas chaves específicas (compatibilidade)
    // Verificar CDX/EVO
    if (in_array($productId, $cdxEvoIds) && isset($productData['pagseguro'])) {
        return true;
    }
    
    // Verificar CDC
    if (in_array($productId, $cdcIds) && isset($productData['brasilcard'])) {
        return true;
    }
...
<div class="pt-6 px-4">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">Edite as informações do estabelecimento</p>
            <p class="text-sm text-gray-500"><?= htmlspecialchars($establishment['nome_fantasia'] ?? '') ?></p>
        </div>
        <div>
            <a href="<?= url('estabelecimentos/' . ($establishment['id'] ?? '')) ?>" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg inline-flex items-center transition-colors">
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <form method="POST" action="<?= url('estabelecimentos/' . ($establishment['id'] ?? '')) ?>" enctype="multipart/form-data" class="p-6">
            <?= csrf_field() ?>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="relative flex cursor-pointer">
                        <input type="radio" name="registration_type" value="PF" class="sr-only peer" required
                               <?= ($establishment['registration_type'] ?? '') === 'PF' ? 'checked' : '' ?>>
                    ...
                    <label class="relative flex cursor-pointer">
                        <input type="radio" name="registration_type" value="PJ" class="sr-only peer" required
                               <?= ($establishment['registration_type'] ?? '') === 'PJ' ? 'checked' : '' ?>>
                        <div class="w-full p-4 border-2 border-gray-300 rounded-lg transition-all duration-200 peer-checked:border-blue-600 peer-checked:bg-blue-600 hover:border-blue-400">
                            <div class="flex items-center justify-center">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <small id="cep-help" class="text-gray-500 <?= ($establishment['registration_type'] ?? '') === 'PF' ? '' : 'hidden' ?>">Digite o CEP e clique em Buscar para preencher automaticamente</small>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="uf" required 
                            ...
                            foreach ($ufs as $uf): ?>
                                <option value="<?= $uf ?>" <?= ($establishment['uf'] ?? '') === $uf ? 'selected' : '' ?>><?= $uf ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    foreach ($products as $product): 
                        ...
                        write_log("Produto ID: {$product['id']}", 'app.log');
                        write_log("Display Name: {$displayName}", 'app.log');
                        write_log("ProductData tem 'other': " . (isset($productData['other']) ? 'SIM (' . count($productData['other']) . ' itens)' : 'NÃO'), 'app.log');
                        write_log("ProductData tem 'brasilcard': " . (isset($productData['brasilcard']) ? 'SIM' : 'NÃO'), 'app.log');
                        write_log("ProductData tem 'pagseguro': " . (isset($productData['pagseguro']) ? 'SIM' : 'NÃO'), 'app.log');
            ...
            <div class="mb-8">
                ...
                <!-- CDX/EVO -->
                <div id="prod-pagseguro-config" class="<?= (isset($productData['pagseguro']) || (isset($productData['other']) && is_array($productData['other']) && array_filter($productData['other'], function($p) { return ($p['product_type'] ?? '') === 'prod-subaquirente'; }))) ? '' : 'hidden' ?> mb-6 p-4 bg-gray-800 rounded-lg border border-gray-700">
                    <h5 class="font-medium text-white mb-3">CDX/EVO</h5>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="previsao_faturamento_prod-pagseguro" id="previsao_faturamento_prod-pagseguro"
                                   value="<?= !empty($productData['pagseguro']['previsao_faturamento']) ? 'R$ ' . number_format((float)$productData['pagseguro']['previsao_faturamento'], 2, ',', '.') : 'R$ 0,00' ?>"
                                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <select name="tabela_prod-pagseguro" 
                                ...
                                foreach ($tabelas as $tabela): ?>
                                    <option value="<?= $tabela ?>" <?= ($productData['pagseguro']['tabela'] ?? '') === $tabela ? 'selected' : '' ?>><?= $tabela ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="meio_pagamento_prod-pagseguro" 
                                ...
                                <option value="a_vista" <?= ($productData['pagseguro']['meio_pagamento'] ?? '') === 'a_vista' ? 'selected' : '' ?>>À Vista</option>
                                <option value="cartao" <?= ($productData['pagseguro']['meio_pagamento'] ?? '') === 'cartao' ? 'selected' : '' ?>>Cartão</option>
                                <option value="criacao" <?= ($productData['pagseguro']['meio_pagamento'] ?? '') === 'criacao' ? 'selected' : '' ?>>Criação</option>
                                <option value="isento" <?= ($productData['pagseguro']['meio_pagamento'] ?? '') === 'isento' ? 'selected' : '' ?>>Isento</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="text" name="valor_prod-pagseguro" id="valor_prod-pagseguro"
                                   value="<?= !empty($productData['pagseguro']['valor']) ? 'R$ ' . number_format((float)$productData['pagseguro']['valor'], 2, ',', '.') : 'R$ 0,00' ?>"
                                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                   placeholder="R$ 0,00" data-mask="currency">
                        </div>
                    </div>
                ...
                <!-- CDC -->
                <div id="prod-brasil-card-config" class="<?= isset($productData['brasilcard']) ? '' : 'hidden' ?> mb-6 p-4 bg-gray-800 rounded-lg border border-gray-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="taxa_prod-brasil-card" 
                                ...
                                <option value="UCRED" <?= ($productData['brasilcard']['financeira'] ?? '') === 'UCRED' ? 'selected' : '' ?>>UCRED</option>
                                <option value="PARCELA FACIL" <?= ($productData['brasilcard']['financeira'] ?? '') === 'PARCELA FACIL' ? 'selected' : '' ?>>PARCELA FACIL</option>
                                <option value="PARCELEX" <?= ($productData['brasilcard']['financeira'] ?? '') === 'PARCELEX' ? 'selected' : '' ?>>PARCELEX</option>
                                <option value="SOU FÁCIL" <?= ($productData['brasilcard']['financeira'] ?? '') === 'SOU FÁCIL' ? 'selected' : '' ?>>SOU FÁCIL</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">Meio de Pagamento da Adesão</label>
                            <select name="meio_pagamento_prod-brasil-card" 
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="">Selecione o meio</option>
                                <option value="a_vista" <?= ($productData['brasilcard']['meio_pagamento'] ?? '') === 'a_vista' ? 'selected' : '' ?>>À Vista</option>
                                <option value="cartao" <?= ($productData['brasilcard']['meio_pagamento'] ?? '') === 'cartao' ? 'selected' : '' ?>>Cartão</option>
                                <option value="criacao" <?= ($productData['brasilcard']['meio_pagamento'] ?? '') === 'criacao' ? 'selected' : '' ?>>Criação</option>
                                <option value="isento" <?= ($productData['brasilcard']['meio_pagamento'] ?? '') === 'isento' ? 'selected' : '' ?>>Isento</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">Valor</label>
                            <input type="text" name="valor_prod-brasil-card" id="valor_prod-brasil-card"
                                   value="<?= !empty($productData['brasilcard']['valor']) ? 'R$ ' . number_format((float)$productData['brasilcard']['valor'], 2, ',', '.') : 'R$ 0,00' ?>"
                                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                   placeholder="R$ 0,00" data-mask="currency">
                        </div>
                    </div>
                ...
                <?php foreach ($products as $product): ?>
                    ...
                    <?php 
                    // Verificar se o produto está selecionado e obter dados
                    $isOtherProductSelected = false;
                    $otherProduct = null;
                    if (isset($productData['other'])) {
                        foreach ($productData['other'] as $op) {
                            if (($op['product_type'] ?? '') === $product['id']) {
                                $isOtherProductSelected = true;
                                $otherProduct = $op;
                                break;
                            }
                        }
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" id="btn-salvar-estabelecimento" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <span id="btn-salvar-texto">Salvar Alterações</span>
                </button>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const productCheckboxesEdit = document.querySelectorAll('input[name="products[]"]');
    productCheckboxesEdit.forEach(function(checkbox) {
        checkbox.addEventListener('change', function() {
            setTimeout(function() {
                document.querySelectorAll('input[data-mask="currency"]').forEach(function(input) {
                    ...
                    if (data.uf) {
                        ...
                        if (selectUf) {
                            ...
                            if (option) {
                                ...
                                // Se não encontrar, usar value direto
                                selectUf.value = data.uf;
                                camposPreenchidos++;
                            }
                        } else if (inputUf) {
